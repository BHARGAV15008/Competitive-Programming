...
def print_temperature_by_date(weather_data, date):
    for forecast in weather_data["list"]:
        if date in forecast["dt_txt"]:
            print(f"Temperature at {forecast['dt_txt']}: {forecast['main']['temp']}°C")
            break
    else:
...
def print_wind_speed_by_date(weather_data, date):
    for forecast in weather_data["list"]:
        if date in forecast["dt_txt"]:
            print(f"Wind Speed at {forecast['dt_txt']}: {forecast['wind']['speed']} m/s")
            break
    else:
        print("No data available for the given date.")
...
def print_pressure_by_date(weather_data, date):
    for forecast in weather_data["list"]:
        if date in forecast["dt_txt"]:
            print(f"Pressure at {forecast['dt_txt']}: {forecast['main']['pressure']} hPa")
            break
    else:
        print("No data available for the given date.")
